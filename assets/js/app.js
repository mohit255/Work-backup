...
//code is create by deepak start

$(document).on("click",".click_to_resent_email",function(){

 var email=$(this).data("id");
 $.ajax({
      method:"POST",
      url:base_url + 'user/dashboard/send_email_dsp',
      data:{"email":email},
      dataTtpw:"html",
      success:function(data){
        ...
        if(data==2)
        {
             $('#register_errtext').html('<div class="account-error success m-t-0">Activation Link has been sent to your email id.</div>');
             // $("#login-popup").modal();
        }
      }
 });
...
// function add_comment_for_blocg($user_id,$blog_id)
// {
...
function selected_menu(value) {
    ...
    window.location=base_url+'signup/'+value;
        ...
        $.ajax({
            ...
            success: function(response) {
                 ...
                 if (response == 0) {
                    ...
                    $('#register_errtext').html('<div class="account-error">Your account is not active if you have not any email <a href="javaacript:;" data-id="'+username+'*#*'+password+'" class="click_to_resent_email" style="left:0 !important; color:#000; " ><u>Click Here</u> </a> to resent activation Link to your email id..</div>');
                ...
                {
  var data_set=response.split("*#*");

                   
                    if (selected_menu != '') {

                       window.location.href = base_url + selected_menu +"/"+data_set[1];                        

                    } else {
                      window.location.href = base_url+"user-profile/"+data_set[0];
...
function check_extra_package(id) {
    ...
    var checkedValues = $("li input[type=checkbox]:checked").map(function() {
      ...
      var sum = 0;
        ...
        if (rate == 0) {
            ...
            rate = parseFloat($('#package_actual_rate_'+id).val()).toFixed(2);
    ...
    var package_id = $('#package_id_'+id).val();
...
console.log(data.watting_data);
			 $("#locloader").css("display", "none");

            $('#extra_gig_calculation').html(data.html);
            $('#set_watting_notic').html(data.watting_data);

            $('#package_rate_set').val(data.package_data["price"]);

            $('#package_actual_rate_set').val(data.package_data["price"]);

            $('#package_id_set').val(data.package_data["id"]);

            $('#currency_type_set').val("$");
            $('#package_finesh_day_set').val(data.package_data["duration"]);
...
function extra_gig_days(e, status) { 
   ...
   $(e).val(value);
        ...
        if (value > main_delivering_time) {
            ...
            $(e).parent().find(".sub_delivery_days").html(main_delivering_time+' Days');

        } else {

            if (value == 1) {

                result = main_delivering_time+' Day';
...
$(document).ready(function(e) {
    ...
    $('#update_gig').submit(function() {
        ...
        if ($.trim(ckValue).length === 0) {
            ...
            $("#desc_err").html('<p>Please enter about your gig details jh vkfdvfbv fdkhm sdvhfdv fdhv fdvg</p>');
  ...
  showAutocompleteOnFocus: true
   ...
   console.log(this.value.length);
        if (value == 1) {
           ...
           $('#main_delivery_days').css("left","40px !important");
            $('#main_delivery_days').html('Day');

        } else if (value > 1) {

            $('#main_delivery_days').html('<font><b style="opacity: 0;">'+this.value+'</b>Days</font>');
