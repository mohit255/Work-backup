...
class Orders extends CI_Controller{
    ...
    public function index($start=0)
    {
        ...
        $this->data['list'] = $this->admin_panel_model->get_orders_list();
    ...
    public function get_one_order()
    {
        ...
        $this->data['data']=$this->admin_panel_model->one_order($id);
         // $this->data['page'] = 'edit'; 
        $this->load->vars($this->data);
        $this->load->view("admin/modules/orders/edit");  
    ...
    public function update_order()
    {
        ...
        $update_addon  = array('status' => $status);
        $this->db->where(array("id"=>$id));
        $update_query = $this->db->update("user_addon_requests",$update_addon);
        if($update_query)
        {
            $user_addon_activate = array("package_id" => $membership_id,"package_status" => "1");
            $this->db->where(array("USERID"=>$user_id));
            $this->db->update("user_login",$user_addon_activate);
             $this->session->set_flashdata('message','User Addon is Updated.');
        }
        else
        {
             $this->session->set_flashdata('alert','Failed To Update User Addon !');
        }
