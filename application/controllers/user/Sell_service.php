...
//mysqli_query($conn,"UPDATE `registration` SET `image`='".$imageName_only."' WHERE id='".$id."'");
    ...
    function getTimezoneGeo($geoplugin_latitude, $geoplugin_longitude,$t)

	 {
		...
		$json = "https://maps.googleapis.com/maps/api/timezone/json?location=20,85&timestamp=1393575206&sensor=false";    
...
public function get_citys()
{
	...
	$citys=$this->db->query("SELECT * FROM `location` WHERE `state` LIKE '".$stat."' ORDER BY `city` ASC")->result_array();
	if($citys)
	{
...
$email_id=$data['email'];
		...
		// var_dump($this->db->last_query());
         ...
         $data_for_eacort_profile["agency_id"]=@$this->input->post("agencyid");
       	 ...
       	 $data_for_eacort_profile["escort_for"]=implode("*#*", @$this->input->post("set_data_escort_for"));


       	 $data_for_eacort_profile["sub_location"] = implode("*#*", @$this->input->post("sub_location"));
...
if($escort_id)
{
	...
	// var_dump($check_id);
     if($check_id)
     {
         ...
         // var_dump($data_for_eacort_profile["sub_location"]);
         //var_dump($this->db->last_query());
     }
     else
     {
...
// var_dump(@$this->input->post("packeg_start_date"));
      ...
      if(!@empty($this->input->post("days_and_date")))
      {
      ...
      // var_dump($this->input->post("days_and_date"));
...
//       var_dump("================================<br>");
// var_dump(date("d-M-Y", strtotime("+$k days",strtotime(@$package_start_date_set))));
//       var_dump("================================<br>");	
     	if(in_array(date("d-M-Y", strtotime("+$k days",strtotime(@$package_start_date_set))), $temp_date))
     	{
      ...
      // $date = date('d-M-Y', strtotime('+'.$d.' days', strtotime(@$temp_date[0])));
      //           $dayname = date('D', strtotime('+'.$d.' days', strtotime(@$temp_date[0])));
...
// var_dump($set_escort_availability);
  ...
  if(@$this->input->post("escort_rate")["duration"] || @$this->input->post("escort_rate")["in_price"] || @$this->input->post("escort_rate")["out_price"])
        {
        	if(@$this->input->post("escort_rate")["duration"] && @$this->input->post("escort_rate")["in_price"] && @$this->input->post("escort_rate")["out_price"])
        	{
        		// var_dump($this->input->post("tour"));
            ...
            $escort_rate["in_price"]=@$this->input->post("escort_rate")["in_price"];
            $escort_rate["out_price"]=@$this->input->post("escort_rate")["out_price"];
...
$this->data["gallery_image"]=$this->db->query("select * from gallery_image where user_id='".$id."' ORDER BY id DESC")->result_array();
 ...
 $this->data["my_escort"]=$this->db->query("select * from escort_info where escort_id!='".$this->session->userdata('SESSION_USER_ID')."' and agency_id='".$this->session->userdata('SESSION_USER_ID')."' ORDER BY id DESC LIMIT 7")->result_array();
...
$this->data["get_my_orders_log"]=$this->db->query("SELECT ua.id AS order_id,ua.user_id,ua.membership_id,ua.order_no,ua.requested_time,ua.status AS order_staus,m.* FROM `user_addon_requests` AS ua,membership AS m WHERE ua.user_id = '".@$id."' AND m.id = ua.membership_id ORDER BY ua.id DESC")->result_array();
$this->load->model('gigs_model');
...
. "<iframe src=\"//player.vimeo.com/video/$vimeo_url?portrait=0&color=333\" width=\"$width\" height=\"$height\" frameborder=\"0\" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>"
 ...
 echo $html;
	...
	public function update_escort_online_status()
    {
       $user_id = $this->session->userdata('SESSION_USER_ID');
	   $id = explode("_", $this->input->get('id'));
	   $status = $id[1];
	   if($status = 1)
	   {
	   $user = $this->db->query("select * from user_login where USERID = '".$user_id."'")->row_array();
	   // var_dump($user["available_till"]);
	   // $current_time = date("Y-m-d H:i A");
	   $current_time = date('Y-m-d H:i:s');
	   $date = new DateTime(date('Y-m-d H:i:s'));
	   $date->modify("+2 hours");
	   $available_till = $date->format('Y-m-d H:i:s');
	   // var_dump($current_time."//".$available_till);
	   if($user["available_till"] < $current_time)
	   {
	   	$this->db->where(array("USERID"=>@$user_id));
        $this->db->update('user_login',array("available_till"=>@$available_till));
	   }
	   }
