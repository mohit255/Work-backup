...
class Dashboard extends CI_Controller{
    ...
    $this->data['currency_option'] = 'USD';
       ...
       // $this->data['latest_gigs'] = $this->gigs_model->latest_gigs();
        ...
        $email_settings = $this->db->get('email_settings')->result_array();
        $config = array(
                'protocol'  => $email_settings[0]['protocol'],
                'smtp_host' => $email_settings[0]['smtp_host'],
                'smtp_crypto' => $email_settings[0]['smtp_crypto'],
                'smtp_port' => $email_settings[0]['smtp_port'],
                'smtpdebug' => $email_settings[0]['smtpdebug'],
                'smtp_user' => $email_settings[0]['smtp_user'],
                'smtp_pass' => $email_settings[0]['smtp_pass'],
                'mailtype'  => $email_settings[0]['mailtype'],
                'charset'   => $email_settings[0]['charset'],
                'priority'  => $email_settings[0]['priority']
            );
...
public function cancel_order(){
  ...
  $cancel_order = $this->db->update('user_addon_requests',array('status'=>3));
  $this->session->set_flashdata('message',"Your Order Canceled.");
  // var_dump($this->db->last_query());
}
public function show_notice_of_get_addon_dsp(){
  $addon_id=$this->input->post("addon_id");
  $user_id=$this->input->post("user_id");
   $data=$this->db->query("select * from user_addon where package_id='".$addon_id."' and user_id='".$user_id."' and status='Running'")->row_array();
   echo "<p>You are already buy this addon. Your Addon end on ".@$data["end_date"]."</p>";
}

public function new_addon_request(){
  date_default_timezone_set('Asia/Calcutta');
  $addon_id=$this->input->post("addon_id");
  $user_id=$this->input->post("user_id");
   $data=$this->db->query("SELECT *  FROM `user_addon_requests` WHERE `user_id` = '".$user_id."' AND status = 0")->row_array();
   if(@!$data){
    $order_no = time();
   $addon_data = array(
    'user_id' => $user_id,
    'membership_id' => $addon_id,
    'order_no' => $order_no,
    'requested_time' => date('Y-m-d H:i:s'),
    'request_date_in_string' => strtotime(date('d-M-Y'))
    );

   $data=$this->db->insert("user_addon_requests",$addon_data);
   echo "<p>Your Request is Successed , Order Number is <b>".@$order_no."</b></p><br><p>Please Deposit the Amount in the EscortsoZ Bank Account and then Mail with transaction receipt at <b>mohit.chack@digimonk.in)</b></p> <br><p><b>Thank You.</b></p>";
   }
   else
   {
    $membership_data =$this->db->query("SELECT * FROM `membership` WHERE `id` = '".$data['membership_id']."'")->row_array();
    @$old_date = $data['requested_time'];
    @$old_date_timestamp = strtotime($old_date);
    // $new_date = date('F d , Y', $old_date_timestamp); 
    @$new_date = date('d-M-Y', $old_date_timestamp); 
    @$new_date__ = date('F d ,Y',strtotime($data['requested_time']));
    echo "<p style='color:red;'>You are already request for <b>(".$membership_data['name'].")</b> addon on ".$new_date." ,<br> Your Order Number is <b>".@$data['order_no'].".</b></p><br><p>Please Deposit the Amount in the EscortsoZ Bank Account and then Mail with transaction receipt at <b>(mohit.chack@digimonk.in).</b></p> <br><p><b>Thank You.</b></p>";
  }
}
...
//     </tr>
   ...
   $total_amount=$commition_amount+$product_price;
     ...
     $get_duration_for_adone=$package_details['duration'];
       ...
       switch ($package_details['duration_type']) {
         case 'DAY':
       
             $duration_count=(int)$get_duration_for_adone*1;
           break;
         case 'WEEK':
           ...
           case 'MONTH':
               $totla_day=0;
                  for($j=0;$j<$package_details['duration'];$j++)
                  {
                    $monat=date("m");
                $year=date("Y");
                    $monat = $monat+$j;
                $days=cal_days_in_month(CAL_GREGORIAN,$monat,$year);
                $totla_day=$totla_day+$days;

                // var_dump($days.",");
                  }
                  // var_dump($totla_day);
               $duration_count=(int)$totla_day;
           break;

           case 'YEAR':
                $duration_count=(int)$get_duration_for_adone*365;
    ...
    if($package_details['values']<=$count_user['count(id)'])
    {
        $set_wation_number=($count_user['count(id)']-$package_details['values'])+1;
          $wating_message='<div class="account-error">Limit of this addon is full Your watting number is '.$set_wation_number.'</div>';
    }  
    else
    {
      $wating_message='';
...
// var_dump($data2);
// var_dump($data2['verified']);
// var_dump("<br>");
$pack_data=$this->db->query("SELECT * FROM membership WHERE types='Addon' AND package = 'Default'");
 $_pack_data=$pack_data->row_array();
// var_dump($_pack_data);
// exit();
 if(@$data2['verified'] != 0)
 {
		if(!empty($username))
      {
        $user_pack_data = array(
          'USERID' => $data2['USERID'], 
          'seller_name' => 'EscortsoZ', 
          'package_id' => $_pack_data['id'], 
          'currency_type' => 'USD', 
          // 'time_zone' => $this->session->userdata('time_zone'), 
          'time_zone' => 'Asia/Calcutta', 
          'item_amount' => $_pack_data['price'], 
          'currency' => 'USD', 
          'commision' => 0, 
          'dollar_amount' => $_pack_data['price'], 
          'created_at' => date('Y-m-d H:i:s'), 
          'created_at_date' => date("d-M-Y H:m:sa"), 
          'created_at_date_in_string' => strtotime(date("d-M-Y")), 
          'package_fininish_date' => date('d-M-Y', strtotime('+3 months', strtotime(date("d-M-Y")))), 
          'package_fininish_date_in_string' => strtotime(date('d-M-Y', strtotime('+3 months', strtotime(date("d-M-Y"))))), 
          'status' => 1, 
          'notification_status' => 1, 
          'notification_paycomplete' => 1, 
          'source' => 'paypal'
        );

        $insert_signup_pack = $this->db->insert("payments",$user_pack_data);
        // var_dump($this->db->last_query());
        $payment_id = $this->db->insert_id();

        // var_dump(date('Y-m-d H:i:s'));

        $user_addon_data = array(
          'payment_id' => $payment_id,
          'user_id' => $data2['USERID'],
          'package_id' => $_pack_data['id'],
          'buy_date' => date("d-M-Y H:m:sa"),
          'buy_date_in_string' => strtotime(date("d-M-Y")),
          'start_date' => date("d-M-Y H:m:sa"),
          'start_date_in_string' => strtotime(date("d-M-Y")),
          'end_date' => date('d-M-Y', strtotime('+3 months', strtotime(date("d-M-Y")))),
          'end_date_in_string' => strtotime(date('d-M-Y', strtotime('+3 months', strtotime(date("d-M-Y"))))),
          'status' => 'Done'
         );
        // var_dump(date("d-M-Y"));
        // var_dump(date('d-M-Y', strtotime('+3 months', strtotime(date("d-M-Y")))));
        $this->db->insert("user_addon",$user_addon_data);

        $after_verify_data['verified'] = 0; 
        // $data['verified'] = 0; 
        $after_verify_data['status'] = 1; 
        
        $after_verify_data['package_id'] = $_pack_data['id']; 
        
        $after_verify_data['payment_id'] = $payment_id; 
        
        $this->db->update('user_login',$after_verify_data,array('username_code'=>$username));
        var_dump($this->db->last_query());

        // var_dump($user_addon_data);
 ...
 {
 		$this->load->view("user/account_activation/failed.php");
 }
...
// $this->load->view("email/send",array("email"=>@$email_id,"subject"=>'Email Conformation'.$this->site_name,"mess"=>$message));
        ...
        $mail_message = $message;
        $this->email->to(@$email_id);
        $this->email->from('mohit.chack@digimonk.in', 'EscortOZ');
