	...
	{ 
       ...
       if($dataname["types"]=="Addon")
       {
        ...
        $get_duration_for_adone=$dataname['duration'];
                  ...
                  switch ($dataname['duration_type']) {
         case 'DAY':
       $days_set=(int)$get_duration_for_adone*1;
           ...
           case 'MONTH':
               $totla_day=0;
                ...
                // var_dump($days.",");
                  ...
                  // var_dump($totla_day);
               $days_set=(int)$totla_day;
           break;

           case 'YEAR':
                $days_set=(int)$get_duration_for_adone*365;
      ...
      else
      {
      	 $days_set=$package_finesh_day_set;
            $finish_date="";    
                   switch ($dataname['duration_type']) {
				         case 'DAY':
				             $days_set=(int)$dataname['duration'];
                             $finish_date = Date('d-M-Y', strtotime("+".$days_set." day"));
				           break;
				         case 'WEEK':
				              $days_set=(int)$dataname['duration']*7;
				              $finish_date=Date('d-M-Y', strtotime("+".$days_set." day"));
				           break;
				           case 'MONTH':
				               $totla_day=0;
				           ...
				           case 'YEAR':
				                 $days_set=(int)$dataname['duration']*365;
				                $finish_date=Date('d-M-Y', strtotime("+".$days_set." day"));
		...
		$data['package_fininish_date'] 				 = $finish_date;	 
$data['package_fininish_date_in_string'] 	 =  strtotime(Date('d-M-Y', strtotime($finish_date)));
      }
...
// var_dump($this->db->last_query());
        ...
        {
        	redirect(base_url()."user/buy_service/paypal_success/?item_number=".$users_tbl_id);
        }
...
$package_preview_link_set_dsp=base_url().'addon/';

 $set_types_dsp=$data_one['package_types'];
}
else
{
	$set_types_dsp=$data_one['package_types'];
	// $package_preview_link=base_url().'price-table-for/'.$data_one['types'];
	$package_preview_link_set_dsp=base_url().'price-table-for/'.$data_one['types'];
		...
		$gig_price = $this->gigs_model->gig_price();
			...
			$body = str_replace('{title}',str_replace("-"," ",$title." ".$set_types_dsp), $body);	


			$body = str_replace('{package_preview_link}',$package_preview_link_set_dsp, $body);
			...
			// $this->email->send();	
$email_id=$toemail;
$this->load->view("email/send",array("email"=>@$email_id,"subject"=>'Buy'." ".$data_one["package_types"]." ".$this->site_name,"mess"=>$seller_message));
		//admin mail function
 ...
 $user_thumb_image=base_url()."assets/images/avatar2.jpg";
    if(@$data_one['user_thumb_image'])
    {
    	 $user_thumb_image=base_url()."assets/uploads/".$data_one['user_thumb_image'];
			...
			$body = str_replace('{ITEM_NAME}',str_replace("-"," ",$title." ".$data_one["package_types"]), $body);
		...
		public function rejected_orders()
	{
       ...
       $get_packeg_data_ditales=$this->db->query("select * from membership where id='".$purchase_details['package_id_set']."'")->row_array();
        ...
        $count_user=$this->db->query("select count(id) from user_addon where package_id='".$purchase_details['package_id_set']."' and (status='Running' or status='Pending')")->row_array();
           ...
           switch ($get_packeg_data_ditales['duration_type']) {
				         case 'DAY':
				             
				             $days_set=(int)$get_packeg_data_ditales['duration'];
                             $finish_date = Date('d-M-Y', strtotime("+".$days_set." day"));
				           break;
				         case 'WEEK':
				              $days_set=(int)$get_packeg_data_ditales['duration']*7;
				              $finish_date=Date('d-M-Y', strtotime("+".$days_set." day"));
				           break;
				           case 'MONTH':
				               $totla_day=0;
				                  for($j=0;$j<$get_packeg_data_ditales['duration'];$j++)
				                  {
				                    $monat=date("m");
				                $year=date("Y");
				                    $monat = $monat+$j;
				           ...
				           case 'YEAR':
				                 $days_set=(int)$get_packeg_data_ditales['duration']*365;
				                $finish_date=Date('d-M-Y', strtotime("+".$days_set." day"));
			...
			$update_user_package_info["end_date"]=$finish_date;
            $update_user_package_info["start_date_in_string"]=strtotime(date("d-M-Y"));
            $update_user_package_info["end_date_in_string"]=strtotime(date('d-M-Y', strtotime($finish_date)));
