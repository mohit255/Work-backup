...
if(@$this->session->userdata("SESSION_USER_ID")!="")
{
  ...
  elseif ($this->uri->segment("4") == 3) {
    $this->session->set_flashdata("already_verified","Account all Ready verified");
  }
  else{
...
$this->load->vars($this->data);   
 ...
 function getTimezoneGeo($geoplugin_latitude, $geoplugin_longitude,$t) {
  ...
  // $response = file_get_contents("https://maps.co.weber.ut.us/arcgis/rest/services/SDE_composite_locator/GeocodeServer/findAddressCandidates?Street=&SingleLine=3042+N+1050+W&outFields=*&outSR=102100&searchExtent=&f=json", false, stream_context_create($arrContextOptions)); echo $response;

    $json = file_get_contents("https://maps.googleapis.com/maps/api/timezone/json?location=$geoplugin_latitude,$geoplugin_longitude&timestamp=$t&key=AIzaSyCrF-ZcLpYjLO7ygnisZJk_eHogmlzawwE ", false, stream_context_create($arrContextOptions));     
