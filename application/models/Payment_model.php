...
$query = $this->db->query("SELECT a.*,b.*,c.* from payments as a,user_login as b,membership as c where c.id=a.package_id and b.USERID=a.USERID and a.USERID='".$id."' ORDER BY a.`created_at` DESC ".$append_sql."");
    ...
    if(!empty($uid)){
           ...
           $query_string = "SELECT dd.*,sg.title,m.username,m.fullname FROM digital_download dd LEFT JOIN sell_gigs sg on sg.id = dd.gig_id LEFT JOIN members m on m.USERID = dd.upload_user_id  where order_id = ".$id." AND  ((seller_id = ".$uid." AND seller_show =0) OR (buyer_id =".$uid."  AND buyer_show = 0))";
