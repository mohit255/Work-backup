...
<?php  if(@$this->session->userdata('are_you')!="Short")
{ ?>
                                 <li class="active">
...
$query = $this->db->query("SELECT id FROM `feedback` WHERE `from_user_id` = $t_uid and `to_user_id` = $f_uid and `gig_id` = $gid and `order_id` = $order_id;");
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                	...
                                                	<a href="javascript:;" <?php if($fead_stautus ==1){?> onclick="add_feedback(<?php echo $f_uid;?> ,<?php echo $t_uid;?>, <?php echo $gid;?>, <?php echo $item['id'];?>);" <?php }else { ?> onclick="add_seller_feedbacks()" <?php  } ?>><?php echo $b_sts;?></a></td>
                                            	...
                                            	<?php if($sts=='Completed Accept') {
												 	...
												 	<?php }else{  ?>
												 		...
												 		<!-- <button type="button" class="btn btn-xs btn-info" <?php if($sts=='Completed Accept') { ?> onclick="change_product_status_update(6,<?php echo $item['id'];?>);" <?php } ?> >Completed</button> -->
                                                    	<!--  --> <div class="dropdown">
												<button class="btn btn-xs btn-info dropdown-toggle dsp_class_set" type="button" id="menu1" data-toggle="dropdown"><?php echo $sts;?>
												<span class="caret"></span></button>



												<ul class="dropdown-menu pull-right" role="menu" aria-labelledby="menu1">
												<li role="presentation"><a role="menuitem" tabindex="-1" href="javascript:;" <?php if($sts=='Completed Accept') { ?> onclick="change_product_status_update(6,<?php echo $item['id'];?>);" <?php } ?> >Completed</a></li>
